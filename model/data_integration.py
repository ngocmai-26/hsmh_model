...
class DataIntegration:
    ...
    def integrate_conduct_data(self):
        ...
        try:
            # Normalize data types - SỬA: semester_year thay vì school_year
            self.data_loader.conduct_df['Student_ID'] = self.data_loader.conduct_df['Student_ID'].astype(str)
            self.data_loader.conduct_df['semester_year'] = self.data_loader.conduct_df['semester_year'].astype(str)
            ...
            self.df['semester_year'] = self.df['year'].astype(str) if 'year' in self.df.columns else '2324'
            self.df['semester'] = '1'  # Default semester nếu không có

            def get_conduct_row(sid, year, semester):
                df = self.data_loader.conduct_df[self.data_loader.conduct_df['Student_ID'] == sid]
                if df.empty:
                    return None
                df = df[(df['semester_year'] < year) | ((df['semester_year'] == year) & (df['semester'] <= semester))]
                if df.empty:
                    return None
                return df.sort_values(['semester_year', 'semester'], ascending=[False, False]).iloc[0]
            ...
            for idx, row in self.df.iterrows():
                ...
                if conduct_row is not None:
                    ...
                    latest_conduct_years.append(conduct_row['semester_year'])
                    latest_classifications.append(conduct_row['student_conduct_classification'])
                    prev = student_conduct[(student_conduct['semester_year'] < conduct_row['semester_year']) | ((student_conduct['semester_year'] == conduct_row['semester_year']) & (student_conduct['semester'] < conduct_row['semester']))]
                    if not prev.empty:
                        prev_row = prev.sort_values(['semester_year', 'semester'], ascending=[False, False]).iloc[0]
    ...
    def integrate_self_study_data(self):
        """Tích hợp dữ liệu tự học - CHỈ merge theo Student_ID và year (không cần semester)"""
        try:
            ...
            tuhoc_df = self.data_loader.tuhoc_df[['Student_ID', 'year', 'accumulated_study_hours', 'accumulated_study_minutes']].copy()
            ...
            # Aggregate total hours and minutes by student and year (KHÔNG group theo semester)
            tuhoc_agg = tuhoc_df.groupby(['Student_ID', 'year']).agg({
                'accumulated_study_hours': 'sum',
                'accumulated_study_minutes': 'sum'
            }).reset_index()
            tuhoc_agg.rename(columns={
                'accumulated_study_hours': 'study_hours_this_year',
                'accumulated_study_minutes': 'study_minutes_this_year'
            }, inplace=True)
            
            # Merge into self.df by Student_ID and year only
            self.df = pd.merge(
                self.df,
                tuhoc_agg,
            ...
            self.df['study_hours_this_year'] = self.df['study_hours_this_year'].fillna(0)
            self.df['study_minutes_this_year'] = self.df['study_minutes_this_year'].fillna(0)
            
            # Tính tổng thời gian tự học (giờ)
            self.df['total_study_hours'] = self.df['study_hours_this_year'] + self.df['study_minutes_this_year'] / 60.0
            
            print(f"✅ Đã tích hợp dữ liệu tự học vào self.df!")
            print(f"   Số sinh viên có data tự học: {(self.df['total_study_hours'] > 0).sum()}")
            print(f"   Trung bình giờ tự học/năm: {self.df['total_study_hours'].mean():.2f}h")
        except Exception as e:
    ...
    def finalize_features(self):
        ...
        # Add self-study features (đổi thành _this_year)
        self_study_features = ['study_hours_this_year', 'study_minutes_this_year', 'total_study_hours']
